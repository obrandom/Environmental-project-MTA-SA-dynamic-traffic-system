...
function cycleNPCs()
	...
	for npc,unsynced in pairs(unsynced_npcs) do
		if getElementHealth(getPedOccupiedVehicle(npc) or npc) >= 1 then
            ...
            -- Limit the 'time jump' to a maximum value of 50ms (equal to update rate).
            -- This prevents NPCs from 'jumping' too much in curves if the server lags.
            time_diff = math.min(time_diff, 50)
